...
public class TeilnahmenService {
  ...
  private final TeilnehmerAnlassLinkMapper teilnehmerAnlassLinkMapper;

  public Collection<OrganisationTeilnahmenStatistikDTO> getAnlassorganisationStati(int jahr,
      Organisation organisation) {
    List<TeilnahmenDTO> teilnahmenPerTeilnehmer = getTeilnahmen(jahr, organisation, false);
  ...
  public List<TeilnahmenDTO> getTeilnahmen(int jahr, Organisation organisation, boolean adjust) {

    Map<Teilnehmer, List<TeilnehmerAnlassLink>> allTeilnehmer = anlassSrv.getTeilnahmen(jahr,
        organisation);
    Pageable pageable = PageRequest.of(0, 500); // return all
    Page<Teilnehmer> teilnehmerListe = teilnehmerSrv.findTeilnehmerByOrganisation(organisation,
        pageable);
    ...
    List<TeilnahmenDTO> teilnahmenDtos = allTeilnehmer.entrySet().stream().map(entry -> {
      List<TeilnehmerAnlassLinkDTO> talDTOList = entry.getValue().stream().map(tal -> {
        ...
        if (!anlassCache.containsKey(talDTO.getAnlassId())) {
          Anlass anlass = anlassSrv.findById(talDTO.getAnlassId());
      ...
      if (entry.getValue().size() == 1) {
        ...
        Optional<TeilnehmerAnlassLink> res = entry.getValue().stream()
            .filter(tal -> !KategorieEnum.KEIN_START.equals(tal.getKategorie()))
            .max((tal1, tal2) -> {
  ...
  private TeilnehmerAnlassLinkDTO adjustMeldeStatus(boolean fromUI, Anlass anlass,
      TeilnehmerAnlassLinkDTO talDto,
      ...
      if (talDto.getMeldeStatus() != null && talDto.getMeldeStatus().length() > 0) {
        ...
        if (talDto.getMeldeStatus().equalsIgnoreCase("Abgemeldet")) {
          ...
          if (anlass.getAnmeldungBeginn().isAfter(LocalDateTime.now())) {
            return talDtoFactory(talDto, MeldeStatusEnum.KEINE_TEILNAHME.name(),
          ...
          if (anlass.getErfassenGeschlossen().isBefore(LocalDateTime.now()) && anlass
            ...
            return talDtoFactory(talDto, MeldeStatusEnum.ABGEMELDET_1.name(),
                talDto.getKategorie());
          }
          ...
          if (anlass.getErfassenGeschlossen().isBefore(LocalDateTime.now())
            ...
            return talDtoFactory(talDto, MeldeStatusEnum.ABGEMELDET_2.name(),
                talDto.getKategorie());
          }
          ...
          if (anlass.getErfassenGeschlossen().isBefore(LocalDateTime.now())
            ...
            return talDtoFactory(talDto, MeldeStatusEnum.ABGEMELDET_3.name(),
                talDto.getKategorie());
          }
          ...
          if (anlass.getStartDate().isBefore(LocalDateTime.now())) {
            return talDtoFactory(talDto, MeldeStatusEnum.ABGEMELDET_4.name(),
                talDto.getKategorie());
          }
        } else {
          if (talDto.getMeldeStatus().equalsIgnoreCase("Startet")) {
            ...
            if (anlass.getErfassenGeschlossen().isBefore(LocalDateTime.now())
              ...
              return talDtoFactory(talDto, MeldeStatusEnum.NEUMELDUNG.name(),
                  talDto.getKategorie());
            }
          }
        }
  ...
  private TeilnehmerAnlassLinkDTO talDtoFactory(TeilnehmerAnlassLinkDTO talDto, String statusText,
    ...
    return new TeilnehmerAnlassLinkDTO(talDto.getAnlassId(), talDto.getTeilnehmerId(),
        talDto.getOrganisationId(),
  ...
  @Transactional
  public TeilnahmenDTO updateTeilnahmen(int jahr, Organisation organisation,
      TeilnahmenDTO teilnahmenDto)
    ...
    List<TeilnehmerAnlassLink> tals = teilnehmerAnlassLinkSrv.findTeilnehmerAnlassLinkByTeilnehmer(
        teilnehmer);
  ...
  private void mergeTeilnehmerAnlassLink(TeilnehmerAnlassLink persisted,
      TeilnehmerAnlassLink talNeu) {
