...
public class AnmeldeKontrolleOutput {
  ...
  public static void csvWriteToWriter(AnmeldeKontrolleDTO anmeldeKontrolle,
      HttpServletResponse response) {
    try {
      csvWriteToWriter(anmeldeKontrolle, response.getOutputStream());
    } catch (Exception e) {
  ...
  public static void csvWriteToWriter(AnmeldeKontrolleDTO anmeldeKontrolle,
    ...
    try {
      ...
      AnmeldeKontrolleTableHeaderEventHandler headerEventHandler = new AnmeldeKontrolleTableHeaderEventHandler(
          doc,
          anmeldeKontrolle);
      ...
      if (anmeldeKontrolle.getVereinsStart() != null
          && anmeldeKontrolle.getVereinsStart().size() > 0) {
        ...
        fillTable(table, fontN, fontB, KategorieEnum.K5B, TiTuEnum.Ti, dto.getTals_K5B_Ti());
        fillTable(table, fontN, fontB, KategorieEnum.K5A, TiTuEnum.Ti, dto.getTals_K5A_Ti());
        fillTable(table, fontN, fontB, KategorieEnum.K5, TiTuEnum.Tu, dto.getTals_K5_Tu());
        ...
        fillTable(table, fontN, fontB, KategorieEnum.KD, TiTuEnum.Ti, dto.getTals_KD_Ti());
        fillTable(table, fontN, fontB, KategorieEnum.KH, TiTuEnum.Tu, dto.getTals_KH_Tu());
  ...
  private static void fillTable(Table table, PdfFont fontN, PdfFont fontB, KategorieEnum kategorie,
      TiTuEnum titu,
  ...
  private static void printCell(Table table, PdfFont font, String value, boolean leftAlign)
      throws IOException {
    Cell cell = new Cell();
  ...
  private static void printTitelCell(Table table, PdfFont font, String value,
  ...
  private static class TextFooterEventHandler implements IEventHandler {
    ...
    public void handleEvent(Event currentEvent) {
      ...
      canvas
          ...
          .showTextAligned(
              "Anlage und Startgerät sind provisorisch und können bis zum Wettkampf ändern",
