...
export class AnlassSummaryEffects {
  ...
  loadAnlassSummaries$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      mergeMap((action) => {
        return this.anlassService.getAnlassOrganisationSummaries(this.authService.currentVerein).pipe(
    ...
    return this.actions$.pipe(
      ...
      mergeMap((action) => {
        ...
        return this.anlassService.updateVereinsStart(newOal).pipe(
          ...
          catchError((error) => {
            return of(AnlassSummariesActions.updateAnlasssummaryError({ error: error }));
          }),
        );
      }),
    );
  ...
  refreshSingleAnlassSummary$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      mergeMap((action) => {
        return this.anlassService.getAnlassOrganisationSummary(action.payload.anlass, action.payload.verein).pipe(
