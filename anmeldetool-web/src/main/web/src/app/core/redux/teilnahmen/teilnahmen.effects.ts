...
export class TeilnahmenEffects {
  ...
  loadTeilnahmen$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      mergeMap((action) => {
        return this.anlassService.getTeilnahmen(this.authService.currentVerein, action.payload).pipe(
          map((teilnahmen) =>
          ...
          catchError((error) => {
            console.log(error);
            return of(TeilnahmenActions.loadAllTeilnahmenError({ error: error }));
          }),
        );
      }),
    );
  ...
  updateTeilnahmen$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      mergeMap((action) => {
        ...
        return this.anlassService.saveTeilnahme(this.authService.currentVerein, action.payload).pipe(
          map((teilnahmen) => {
  ...
  deleteTeilnehmer$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      mergeMap((action) => {
        return this.teilnehmerService.delete(this.authService.currentVerein, action.payload).pipe(
          ...
          catchError((error) => {
            return of(TeilnahmenActions.deleteTeilnehmerError({ error: error }));
          }),
        );
      }),
    );
