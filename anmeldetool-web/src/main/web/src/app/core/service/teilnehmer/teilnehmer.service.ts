...
export class TeilnehmerService extends ServiceHelper {
  ...
  getTeilnehmer(verein: IVerein, pageIndex = 0, pageSize = 150): Observable<ITeilnehmer[]> {
  ...
  add(verein: IVerein, titu: TiTuEnum): Observable<ITeilnehmer> {
    ...
    const tiTuKey = (Object.keys(TiTuEnum) as Array<keyof typeof TiTuEnum>).find((key) => TiTuEnum[key] === titu);
    const teilnehmer: ITeilnehmer = {
      // Persist enum key shape expected by ITeilnehmer.
      tiTu: tiTuKey,
    };
    return this.addTeilnehmer(verein, teilnehmer);
  }

  addTeilnehmer(verein: IVerein, teilnehmer: ITeilnehmer): Observable<ITeilnehmer> {
