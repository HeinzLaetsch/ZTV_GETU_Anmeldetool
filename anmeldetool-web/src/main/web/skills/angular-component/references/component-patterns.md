...
// Usage: <lxt-slider [(value)]="sliderValue" />
```
...
// Parent
@Component({
  template: `<lxt-child [data]="parentData()" [config]="config" />`,
})
export class Parent {
...
// Parent
@Component({
  template: `<lxt-child (saved)="onSaved($event)" />`,
})
export class Parent {
  onSaved(data: Data) {
...
@Component({
  template: `
    @defer (on viewport) {
      <lxt-heavy-chart [data]="chartData()" />
    } @placeholder {
...
@Component({
  imports: [Highlight],
  template: `<lxt-card appHighlight="lightblue" />`,
})
export class Page {}
```
