...
import { initialState, loadingAdapter } from './busy-indicator-progress-bar.state';
...
export const loadingFeature = createFeature({
  ...
  reducer: createReducer(
    ...
    on(LoadingActions.loadingEventProcessed, (state, action) => {
      console.log('LoadingEvent Store element removed processed: ', state, ' ', action);
...
export const { selectAll, selectEntities, selectIds, selectTotal } = loadingAdapter.getSelectors();
